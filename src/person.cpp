...
Person::Person(int id, std::string name, std::chrono::year_month_day birthDate,
...
int Person::getAge() {
  ...
  // eliminating the time of day
  const auto now = floor<days>(system_clock::now());

  // converts the time_point now into the type year_month_day, and then extracts
  // only the year.
